...
export function Docker({ ...props }) {
  ...
  return (
    <Card className="col-span-8">
      <CardHeader>
        <CardTitle>Docker containers ({observation.docker.containers.length})</CardTitle>
      </CardHeader>
          ...
          <TableBody>
            {(observation.docker.containers ?? []).map((container: any) => (
              <TableRow key={container.image}>
                ...
                <TableCell className="text-xs">{container.restarting ? "Yes" : "No"}</TableCell>
                <TableCell className="text-xs">{container.started_at}</TableCell>
                <TableCell className="text-xs">{container.finished_at}</TableCell>
